...
if(!empty($_POST)){
  if($_POST['message'] !== ''){
    ...
    member_id=?, message=?,reply_message_id=?, created=NOW() ');
...
if(isset($_REQUEST['res'])){
  // Reply
  $response = $db->prepare('SELECT m.name, m.picture, p.* FROM members m, posts p WHERE
  m.id=p.member_id AND p.id=?');
...
<?php foreach($posts as $post): ?>
    ...
    <p class="day"><a href="view.php?id=<?php print(htmlspecialchars($post['id'], ENT_QUOTES)); ?>"><?php 
    print(htmlspecialchars($post['created'], ENT_QUOTES)); ?></a>
